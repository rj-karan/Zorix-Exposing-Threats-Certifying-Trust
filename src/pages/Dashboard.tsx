...
const STATS = [
  { label: 'Active Verifications', value: 12, sub: '+3 today', color: '#e8001d' },
  { label: 'Pending Analysis',     value: 7,  sub: 'In queue', color: '#ffc800' },
  { label: 'Approved & Patched',   value: 34, sub: '↑ 8 this week', color: '#00ff64' },
  { label: 'Rejected / False+',    value: 6,  sub: '2 reviewed', color: '#ff7800' },
]

const CVES = [
  { id:'CVE-2024-1234', severity:'CRITICAL', score:9.8, status:'analyzing',  system:'Apache Struts',  time:'2m ago' },
  { id:'CVE-2024-5678', severity:'HIGH',     score:8.1, status:'simulating', system:'OpenSSL 3.x',    time:'11m ago' },
  { id:'CVE-2024-9012', severity:'HIGH',     score:7.6, status:'complete',   system:'Linux Kernel',   time:'34m ago' },
  { id:'CVE-2024-3456', severity:'MEDIUM',   score:6.2, status:'failed',     system:'Node.js 18',     time:'1h ago' },
  { id:'CVE-2024-7890', severity:'CRITICAL', score:9.1, status:'complete',   system:'Spring Boot',    time:'2h ago' },
]
            ...
            <tbody>
              {CVES.map((c,i) => (
                <tr key={i} style={{ cursor:'pointer' }}>
                  <td style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:12, color:'#e8001d' }}>{c.id}</td>
                  <td><span style={{ color:SEVERITY_COL[c.severity], fontFamily:"'Share Tech Mono',monospace", fontSize:10, fontWeight:700 }}>{c.severity}</span></td>
                  <td style={{ fontFamily:"'Share Tech Mono',monospace", color:'#f0f0f0' }}>{c.score}</td>
                  <td style={{ color:'#ccc' }}>{c.system}</td>
                  <td><span className={`badge ${c.status}`}><span className="badge-dot"/>{c.status.toUpperCase()}</span></td>
                  <td style={{ fontFamily:"'Share Tech Mono',monospace", fontSize:11, color:'#555' }}>{c.time}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <div className="panel fade-up d4">
          ...
          <div style={{ marginTop:18 }}>
            <button className="btn-solid" style={{ width:'100%' }}>▶ RUN SIMULATION</button>
          </div>
        </div>
      </div>
    </div>
